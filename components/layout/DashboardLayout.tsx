          ...
          <div className="flex justify-between items-center h-16">
            <div className="flex items-center">
              ...
              <span className="ml-4 text-sm font-medium text-gray-500 border-l pl-4 border-gray-300">
                {user?.role ? `${user.role.charAt(0).toUpperCase()}${user.role.slice(1)} Panel` : 'Panel'}
              </span>
            </div>
            <div className="flex items-center">
              <span className="mr-4">Welcome, <span className="font-semibold">{user?.name || 'User'}</span></span>
              <button
                onClick={logout}
